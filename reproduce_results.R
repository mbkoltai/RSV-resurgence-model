...
partable <- list(exp_dep=seq(1/4,2,1/8),
...
l_delta_susc <- 1:nrow(partable) %>%
  lapply( function(n_p) {
      sapply(1:n_age, function(x) {
      (1 * exp(-partable$exp_dep[n_p] * (1:3))) / (exp(partable$age_dep[n_p] * x)) }) } )
# create partable with scaling parameters
partable <- partable %>% 
...
# You can go down to RESULTS and read in the results from the parameter sampling already
#
...
all_sum_inf_epiyear_age_filtered <- left_join(results_summ_all, 
      estim_attack_rates %>% 
        ...
        filter(sum(attack_rate_check)>=n_age*n_sel_yr*check_crit & 
                 sum(seas_share_check)>=n_age*n_sel_yr*check_crit ) %>%
        select(!c(attack_rate_check,seas_share_check,final)) %>% 
        relocate(par_id,.before=epi_year)
# this leads to 1046 parameter sets:
...
# write_csv(partable_filtered_AR_seasconc,here::here(foldername,"partable_filtered_AR_seasconc.csv"))
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
...
epiyear_means <- results_summ_all %>% filter(epi_year<2021) %>% 
  ...
  mutate(inf_in_seas=inf_in_seas/inf_in_seas[epi_year==2019]) %>% 
  group_by(epi_year,agegroup_name) %>% 
  summarise(inf_in_seas=mean(inf_in_seas))
# plot
ggplot() + geom_jitter(data=results_summ_all %>% filter(epi_year<2021) %>% 
  ...
  scale_y_log10() + theme(legend.position="top")+theme_bw() + standard_theme
# save
# ggsave(here::here(foldername,"2020_RSV_check_log.png"),width=32,height=20,units="cm")
...
# Eg. 1st infections for adults are negligible, so inter-year differences in these small numbers 
# we don't take into account)
summ_diff_interyr <- left_join(read_csv(here::here(foldername,"summ_diff_interyr_all.csv")) %>% 
...
# save
# ggsave(here::here(foldername,"interyear_difference_cumul_incid_reg_dyn_LOLZ.png"),width=30,height=25,units="cm")
##############################################################
...
simul_hosp <- left_join(results_summ_all %>% 
                  ...
                  mutate(hosp_num_SIMUL_inf_tot=prob_hosp_per_infection_adj*inf_tot,
                         hosp_num_SIMUL_inf_seas=prob_hosp_per_infection_adj*inf_in_seas) %>% ungroup() %>% 
...
results_summ_all_hosp <- left_join(
  left_join(
      left_join(results_summ_all %>% 
                  ...
                  hosp_probabilities %>% 
                  select(c(agegroup_name,prob_hosp_per_infection_adj)),by="agegroup_name"),
  ...
  for (k_age_excl in 1:2) {
    ...
    p <- ggplot(df_plot, 
      ...
      geom_linerange(aes(ymin=ci50_low,ymax=ci50_up),
                     position=position_dodge(width=dodge_val),alpha=0.6,size=12*size_adj)+
  ...
  for (k_plot_par in 1:length(sel_pars)) {
    for (k_plot_vartype in 1:length(sel_vartypes)) {
      ...
      p <- ggplot(df_plot,
        ...
        geom_vline(xintercept=(0:4)+1/2,size=1/5) + labs(color=unique(df_plot$parname)) +
        scale_x_discrete(expand=expansion(0.02,0)) + 
        xlab("") + ylab(ylab_tag) + theme_bw() + standard_theme + 
...
if (n_par>3) {p <- p + scale_color_manual(values=colorpal) + scale_fill_manual(values=colorpal)}; p 
  ...
  plot_fn <- here::here(foldername,
                        paste0("dynamics/weekly_hosp_by_",ifelse(grepl("exp",k_par),"age_exp",k_par),
...
# for instance looking at dynamics of individual simulations
### THE END :)
#