...
source(here::here("load_params.R")); foldername <- "repo_data"
# estimated attack rates
estim_attack_rates <- read_csv(here::here(foldername, "estim_attack_rates.csv"))
...
source(here::here("fcns", "calc_hosp_rates.R"))
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
...
age_exp_dep_uniqvals <- age_exp_dep_uniqvals %>% 
  mutate(const_delta = 1/unlist(lapply(lapply(1:nrow(age_exp_dep_uniqvals), 
     function(n_p) {sapply(1:n_age, function(x) {(1*exp(-age_exp_dep_uniqvals$exp_dep[n_p]*(1:3)))/(exp(age_exp_dep_uniqvals$age_dep[n_p]*x))})} ), 
...
partable_filename <- here::here("repo_data", "partable_filtered.csv")
n_row <- nrow(read_csv(here::here("repo_data", "partable_filtered.csv")))
...
command_print_runs <- paste0(c("Rscript fcns/write_run_file.R",
  n_core, n_row, simul_length_yr, n_post_npi_yr, partable_filename,
...
results_summ_all <- read_csv(here::here(foldername, "results_summ_all.csv"))
...
all_sum_inf_epiyear_age_filtered <- left_join(
    results_summ_all %>% filter(epi_year %in% sel_yrs),
  ...
  select(!c(median_est,min_est, max_est, median_all_inf, min_est_all_inf, max_est_all_inf, 
    attack_rate_check, seas_share_check,
...
estim_rates <- estim_attack_rates %>%
  select(agegroup_name, median_est, min_est, max_est) %>% 
  pivot_longer(!c(agegroup_name)) %>% 
  rename(type = name) %>%
  mutate(name = "attack_rate_perc")
...
all_sum_inf_epiyear_age_filtered %>%
  mutate(attack_rate_perc = ifelse(epi_year == 2020, NA, attack_rate_perc),
    agegroup_name = factor(agegroup_name, levels = unique(agegroup_name))) %>%
    ungroup() %>% 
    select(c(par_id, epi_year, agegroup_name, attack_rate_perc, seas_share, max_incid_week, exp_dep, age_dep, seasforc_width_wks, R0)) %>% 
  ...
  ggplot() +
    ...
    facet_grid(name~agegroup_name, scales="free_y") +
    ...
    xlab("age-dependence") + ylab("") + theme(legend.position="top") +
    theme_bw() + standard_theme + labs(color=color_var)
# save
...
summ_diff_interyr <-
  left_join(
    read_csv(here::here(foldername, "summ_diff_interyr_reg_dyn.csv")) %>% 
        mutate(
          par_id_sort = as.numeric(factor(par_id))),
    rsv_age_groups %>%
...
right_join(
  ...
  summ_diff_interyr %>%
    ...
    select(c(agegroup,infection)),
  by = c("agegroup","infection")) %>%
  ggplot(aes(sum_rel_diff)) +
    ...
    ylab("CDF") + labs(color="# infection") + scale_x_log10() +
    geom_vline(xintercept = 1/10,linetype = "dashed", size=1/2) + theme_bw() + standard_theme +
...
# save
ggsave(here::here(foldername, "interyear_difference_cumul_incid_reg_dyn.png"), width=30, height=25, units="cm")
##############################################################
# select the parameter sets with less than x% inter-year variation in cumul incid
parsets_regular_dyn <- right_join(
    summ_diff_interyr,
    summ_diff_interyr %>% group_by(agegroup, infection) %>% summarise(attack_rate = round(mean(attack_rate, na.rm=T), 3)) %>% 
    filter(attack_rate > 0.01) %>% select(c(agegroup,infection)),
    by = c("agegroup","infection")) %>% 
  group_by(par_id) %>% 
  summarise(score_reg_dyn = sum(sum_rel_diff < 0.2)) %>%
...
simul_hosp <- left_join(
  results_summ_all %>% filter(par_id %in% parsets_regular_dyn$par_id & epi_year == 2019),
  hosp_probabilities,
  by = "agegroup_name") %>% 
  ...
  ungroup() %>%
  select(c(agegroup_name, hosp_num_from_per_inf_prob, hosp_num_SIMUL_inf_tot, hosp_num_SIMUL_inf_seas, par_id)) %>% 
...
simul_hosp %>% mutate(line_size = ifelse(grepl("per_inf", name), 1/5, 2)) %>%
  ggplot(aes(x = agegroup_name, y = ifelse(value > 0, value / 1e3, NA), color = name)) + 
    geom_hpline(size = 1, width = 0.32, position = position_dodge(width = 1)) +
    geom_vline(xintercept = (0:11) + 1/2, linetype="dashed", size = 1/2) + 
    xlab("Age Group") +
    ylab("thousand hospitalisations in season/epi year") + #scale_x_continuous(expand=expansion(0,0),breaks=1:11)+
...
attack_rates_simul_LIT <- left_join(
  results_summ_all %>%
    ...
    select(agegroup_name, par_id, inf_tot, inf_in_seas),
  data.frame(
    agegroup_name = rsv_age_groups$agegroup_name,
    ...
    cumul_inf_LIT_ESTIM_max = rsv_age_groups$value * estim_attack_rates$max_est/100),
  by = "agegroup_name") %>% 
  mutate(agegroup_name = factor(agegroup_name, levels = unique(agegroup_name))) %>%
  pivot_longer(!c(agegroup_name, par_id)) %>% 
  ...
  mutate(name = case_when(
    name %in% "inf_in_seas_SIMUL" ~ "in-season cumulative incidence (SIMULATION)",
    name %in% "LIT_estim" ~ "in-season cumulative incidence (LITERATURE ESTIMATE)"))

simul_median_cumul_inf <- attack_rates_simul_LIT %>%
  filter(name %in% "in-season cumulative incidence (SIMULATION)") %>%
  group_by(agegroup_name) %>%
  summarise(value = median(value))

# SI FIGURE 2
attack_rates_simul_LIT %>%
  filter(name %in% "in-season cumulative incidence (SIMULATION)") %>%
  ggplot(aes(x = agegroup_name, y = ifelse(value > 0, value/1e3, NA), color = name)) +
    geom_hpline(size = 1/5, width = 0.47, color="red") + # ,position=position_dodge(width=1)
    geom_hpline(
      data=attack_rates_simul_LIT %>%
        filter(grepl("LIT",name)) %>%
        group_by(agegroup_name) %>%
        arrange(value) %>% 
        mutate(min_med_max = row_number()),
      aes(linetype = ifelse(min_med_max == 2, "solid", "dashed")),
        size = 1, width = 1,color = "black") + # position=position_dodge(width=1),
    geom_hpline(
      data = simul_median_cumul_inf,
    ...
    xlab("Age Group") + ylab("thousand cases/season") + 
    scale_y_log10(breaks = round(10^seq(-2,4,by=1/4)), expand = expansion(0.02,0)) + 
...
partable_regular_dyn <- parsets_regular_dyn %>% select(!score_reg_dyn) %>% left_join(partable, by = "par_id")
pred_pca <- data.frame(
  predict(
    prcomp(partable_regular_dyn %>% select(c(exp_dep,age_dep))),
...
pred_pca %>%
  ...
  ggplot(aes(x = PC1, y = value)) +
    ...
    scale_x_continuous(breaks=(-(2*3):(2*2))/4,limits=c(-1,1)) + theme_bw() + standard_theme +
...
suscept_sel_parsets <- left_join(
  ...
  age_exp_dep_uniqvals,
  by=c("exp_dep", "age_dep")) %>%
  rename(
    `exposure-dependence` = exp_dep,
    `age-dependence` = age_dep) %>%
  ...
  group_by(PC1_grouped) %>% mutate(PC1_grouped = round(mean(PC1), 1)) %>%
...
ggplot(suscept_sel_parsets) +
  geom_line(aes(x = age, color = factor(PC1_grouped), group = PC1_grouped, y = med_val), size = 1.06) +
  facet_wrap(~exposure, labeller = labeller(exposure=label_both)) +
  labs(color = "exposure (-1) <-> age (1)") + scale_color_manual(values = colorpal)+
  scale_y_log10() + theme_bw() + standard_theme +
...
suscept_sel_parsets %>%
  rename(kappa=PC1_grouped) %>%
  ungroup() %>%
  mutate(kappa_num = as.numeric(factor(as.character(kappa)))) %>% 
  filter(kappa_num %in% c(1,3,5,7,9,10)) %>%
  ggplot(aes(x = age,color = factor(exposure), group = exposure, fill = factor(exposure))) +
    ...
    scale_y_log10() + theme_bw() + standard_theme +
    theme(
      strip.text=element_text(size = 18), legend.title = element_text(size = 16), legend.text = element_text(size = 15),
      legend.position="top", axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 15),
...
results_summ_all_hosp <- left_join(
  left_join(
    left_join(results_summ_all %>% filter(par_id %in% partable_regular_dyn$par_id), 
      hosp_probabilities %>% select(c(agegroup_name,prob_hosp_per_infection_adj)), by = "agegroup_name"),
    pred_pca %>% select(par_id, PC1),
    by = "par_id"),
  partable_regular_dyn %>% select(par_id, omega) %>% mutate(omega = 1/omega) %>% rename(waning = omega),
  by = "par_id") %>% 
  mutate(hosp_tot = prob_hosp_per_infection_adj * inf_tot,
    hosp_seas = prob_hosp_per_infection_adj * inf_in_seas) %>%
    select(!prob_hosp_per_infection_adj) %>%
    select(!c(median_est, min_est, max_est, median_all_inf, min_est_all_inf, max_est_all_inf,
            attack_rate_check, seas_share_check, max_incid_week_check, final)) %>%
  ...
  relocate(par_id, .after = epi_year) %>% relocate(c(exp_dep, age_dep, PC1, seasforce_peak, R0, waning, seasforc_width_wks), 
    .after = par_id)
...
summ_broad_age_groups <- parsets_broad_age_groups %>% 
  mutate(value_norm = 
    ifelse(name %in% c("seas_share"), value_norm * 100, value_norm)) %>% 
...
summ_broad_age_groups_byvalue <- parsets_broad_age_groups %>%
  ...
  summarise(mean = mean(value_norm),
    ...
    ci95_up = quantile(value_norm, c(0.025, 0.975))[2]) %>%
  filter(epi_year > 2019)
# write_csv(sum m_broad_age_groups_byvalue,paste0(foldername,"summ_broad_age_groups_byvalue.csv"))
...
for (k_plot in 1:length(sel_vars)) {
  for (k_age_excl in 1:2) {
    ...
    df_plot <- summ_broad_age_groups %>%
      mutate(name = case_when(grepl("attack",name) ~ "attack rate",
    ...
    p <- df_plot %>% 
      ggplot(aes(x = factor(epi_year),color = factor(agegroup_broad), group = agegroup_broad)) + 
        ...
        geom_vline(xintercept = (0:4)+1/2, size = 1/2) + scale_x_discrete(expand=expansion(0,0)) + 
        xlab("") + ylab(paste0(sel_vars[k_plot], ylab_tag)) +
        labs(color="age groups") + theme_bw() + standard_theme +
...
for (k_plot_var in 1:length(sel_vars)) {
  for (k_plot_par in 1:length(sel_pars)) {
    ...
    ylab_tag <- ifelse(grepl("season peak|cases in-season|attack", sel_vars[k_plot_var]), " (change from 2019 level)", " (relative to pre-NPI)")
    ...
    p <- df_plot %>% 
      ggplot(aes(x = factor(epi_year), color = factor(parvalue), group = parvalue)) + 
      ...
      scale_x_discrete(expand = expansion(0.02,0)) + xlab("") +
      ylab(paste0(sel_vars[k_plot_var],ylab_tag)) + 
    ...
    if (grepl("season peak|cases in-season|attack", sel_vars[k_plot_var])) { #  # + scale_y_log10() 
      ...
      p <- p + geom_hline(yintercept = 0,linetype = "dashed", size = 1/2) +
        scale_y_continuous(breaks = break_vals) } else {
...
parsets_mean_age_inf <-
  left_join(
    results_summ_all_hosp, 
    rsv_age_groups %>% select(agegroup_name,mean_age_weighted),
    by="agegroup_name") %>% 
  ...
  pivot_longer(!c(par_id, epi_year, PC1,s easforce_peak, waning, seasforc_width_wks, R0)) %>%
...
for (k_plot in 1:length(sel_vars)) {
  dodge_val=0.9; df_plot <- summ_mean_age_infs %>% filter(epi_year>2020 & (name %in% sel_vars[k_plot])) %>% 
    mutate(name = case_when(
      grepl("mean_age_hosp_seas_under_5", name) ~ "mean age of in-season hospitalisations", 
       grepl("mean_age_hosp_tot_under_5", name) ~ "mean age of cumulative hospitalisations"))
  df_plot %>% 
    ggplot(aes(x=factor(epi_year))) + 
...
for (k_plot_var in 1:length(sel_vars)) {
  for (k_plot_par in 1:length(sel_pars)) {
    ...
    df_plot <- summ_mean_age_inf_byvalue %>%
      ...
      mutate(varname = 
        case_when(grepl("mean_age_hosp_seas_under_5",varname) ~ "mean age of in-season hospitalisations",
    ...
    df_plot %>% 
      ggplot(aes(x = factor(epi_year), color = factor(parvalue), group = parvalue)) + 
        ...
        xlab("") + ylab(paste0(unique(df_plot$varname)," (change from 2019 level)")) + 
        scale_y_continuous(breaks = -10:10) + theme_bw() + 
        standard_theme +
        theme(strip.text = element_text(size=15),
          axis.text.x = element_text(size=14),
          axis.text.y=element_text(size=14),
...
hosp_sum_prob_broad_agegr <- hosp_probabilities %>%
  mutate(agegroup_broad = c("<1y","1-2y","2-5y","5+y")[findInterval(factor(agegroup_name, levels = unique(agegroup_name)) ,c(2,4,7) + 1) + 1]) %>%
  group_by(agegroup_broad) %>% 
  summarise(hosp_sum = sum(hosp_num_from_per_inf_prob)) %>% 
...
dyn_all_parsets_broad_age <-
  left_join(
    bind_rows(
      lapply(
        list.files(here::here(foldername), pattern = "dyn_parsets.*csv"), 
          function(x) read_csv(file = paste0(foldername, x)) %>% filter(par_id %in% parsets_regular_dyn$par_id) %>% 
            mutate(date = t - min(t) + as.Date(start_date_dyn_save)) %>%
            select(!c(t,name)) %>% 
            filter(date >= as.Date("2018-10-01") & date <= as.Date("2024-04-01")) %>%
            group_by(agegroup, date, par_id) %>%
            summarise(value = sum(value)) %>%
            group_by(par_id,agegroup) %>%
            mutate(value = round(roll_sum(value, n=7, fill=NA, align="right", by=7))) %>%
            filter(!is.na(value))  )  ), 
    hosp_probabilities %>%
      mutate(agegroup = as.numeric(factor(agegroup_name, levels = unique(agegroup_name)))) %>%
      select(agegroup, prob_hosp_per_infection_adj),
    by="agegroup") %>% 
  mutate(
    incid_hosp = value * prob_hosp_per_infection_adj,
    agegroup_broad = c("<1y","1-2y","2-5y","5+y")[findInterval(agegroup,c(2,4,7)+1)+1]) %>% 
  group_by(agegroup_broad,date,par_id) %>%
  summarise(value = sum(value), incid_hosp = sum(incid_hosp))

# summary by year
summ_dyn_max_incid_seas_length <-
  dyn_all_parsets_broad_age %>%
    ...
    group_by(par_id, agegroup_broad, name) %>%
    mutate(seas_tot_2018 = min(seas_tot_2018, na.rm=T)) %>%
    ungroup() %>%
    mutate(above_baseline = value > seas_tot_2018/52) %>%
    group_by(epi_year, par_id, agegroup_broad,name) %>%
    summarise(max_value = max(value), sum_value = sum(value), seas_length_wk = sum(above_baseline)) %>%
...
summ_max_incid_seas_length_byvalue <- parsets_max_incid_seas_length %>%
  mutate(waning = round(waning)) %>%
  select(!c(PC1, value)) %>% 
  relocate(age_exp_par_bins, .after=R0) %>%
    ...
    for (k_plot_vartype in 1:length(sel_vartypes)) {
      ...
      df_plot <- summ_max_incid_seas_length_byvalue %>% 
        ...
        mutate(varname = case_when(
            grepl("incid_case", varname) ~ "cases", 
            grepl("incid_hosp", varname) ~ "hospitalisations"),
          vartype = case_when(
            grepl("max_value", vartype) ~ "peak", 
      ...
      if (!grepl("age_exp_par_bins",sel_par)){ 
        colorpal=colorRampPalette(colors=c("orange","red"))(n_par_value)
      } else  {
        colorpal=colorRampPalette(colors=c("blue","grey","red"))(n_par_value) 
      }
      p <- df_plot %>% 
      ...
      if (!dir.exists(paste0(foldername,subfldr_name))) {
        dir.create(paste0(foldername,subfldr_name))
      }
      plot_filename <- paste0(foldername, subfldr_name, "summ_stats_",
                          paste0(c(sel_vars[k_plot_var], sel_vartype), collapse="_"), 
                        "_",sel_par,".png")
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
cumul_peak_meanage_hosp_byvalue <- 
  bind_rows(
  # cumulative hospitalisations
    parsets_broad_age_groups %>%
      filter(name %in% "hosp_tot" & epi_year > 2020) %>%
      ungroup() %>%
      rename(varname = name) %>% 
      select(!c(value,PC1,par_id)) %>%
      ...
      summarise(mean_val = mean(value_norm),
        med_val = median(value_norm),
  ...
  # peak hospitalisations
    parsets_max_incid_seas_length %>%
      filter(varname %in% "incid_hosp" & vartype %in% "max_value" & epi_year > 2020) %>%
      ungroup() %>% 
      select(!c(value,PC1,par_id)) %>%
      pivot_longer(c(age_exp_par_bins, seasforce_peak, waning, seasforc_width_wks, R0)) %>% 
      rename(parname=name, parvalue=value) %>%
      relocate(value_norm, .after=parvalue) %>%
      group_by(agegroup_broad, epi_year, parname, varname) %>%
      summarise(mean_val = mean(value_norm),
        med_val = median(value_norm),
        min_val = mean(value_norm[parvalue == min(parvalue)]),
        max_val = mean(value_norm[parvalue == max(parvalue)])),
    # mean age
    parsets_mean_age_inf %>%
      filter(name %in% "mean_age_hosp_tot_under_5" & epi_year > 2020) %>%
      ungroup() %>%
      select(!c(value,PC1,par_id)) %>%
      rename(varname=name) %>%
      pivot_longer(c(age_exp_par_bins,seasforce_peak,waning,seasforc_width_wks,R0)) %>%
...
p_cumul_peak_summ <- cumul_peak_meanage_hosp_byvalue %>%
  mutate(epi_year = as.character(epi_year)) %>%
  filter(epi_year < 2023 & (!parname %in% "seasforc_width_wks") & !is.na(agegroup_broad)) %>%
  mutate(
    parname = case_when(grepl("age_exp_par_bins", parname) ~ "exposure (-1) <-> age (1)",
      grepl("seasforce_peak", parname) ~ "seasonal forcing",
      grepl("R0", parname) ~ "R0 (baseline)",
      grepl("waning", parname) ~ "waning rate"),
    varname = case_when(grepl("hosp_tot", varname) ~ "cumulative hospitalisations",
      grepl("incid_hosp",varname) ~ "peak hospitalisations"),
    epi_year = case_when(epi_year %in% "2021" ~ "2021-2022", epi_year %in% "2022" ~ "2022-2023")) %>%
  ggplot(aes(x = mean_val, y = factor(epi_year))) + # parname
    ...
    facet_grid(agegroup_broad~varname, scales="free_x") +
    geom_vline(xintercept = 1, size = 1/3, linetype="dashed") + 
    ...
    theme(strip.text = element_text(size=18),
      ...
      legend.position = "top",
      axis.title.x = element_text(size=16)) + 
    xlab("proportion of 2019-2020 value") + ylab("") + labs(color="");
p_cumul_peak_summ
# SAVE
...
p_aver_age_shift_summ <- 
  ...
  ggplot(aes(x = mean_val, y = factor(epi_year))) + # parname
    ...
    geom_hline(yintercept = (0:2) + 1/2, size = 1/2) + scale_x_continuous(breaks = -3:5) +
    scale_y_discrete(expand = expansion(0,0)) +
    theme_bw() + standard_theme +
    ...
    xlab("shift in average age of hospitalisations (months)") + ylab("")+ labs(color="")
p_aver_age_shift_summ
...
# save
ggsave(here::here(foldername, "median_interquant_by_param_value/comb_summary_plot_4params_1col.png"), width=40, height=32, units="cm")

### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
# Evaluating the dynamics as a function of parameters
summ_dyn_all_parsets_broad_age <- left_join(
    dyn_all_parsets_broad_age, 
  ...
  group_by(agegroup_broad, date, age_exp_par_bins, name) %>%
  summarise(mean = mean(value), median = median(value),
    ci50_low = quantile(value, c(0.25,0.75))[1],
    ci50_up = quantile(value, c(0.25,0.75))[2],
    ci95_low = quantile(value, c(0.025,0.975))[1],
    ci95_up = quantile(value, c(0.025,0.975))[2]) %>%
  rename(varname = name) %>% 
  pivot_longer(c(mean, median, ci50_low, ci50_up, ci95_low, ci95_up)) %>% 
  mutate(epi_year = ifelse(week(date) >= 42, year(date), year(date)-1)) %>% 
  rename(metric = name) %>%
  group_by(agegroup_broad, age_exp_par_bins, varname,metric) %>% # n_year=year(date)
  mutate(peak_2019 = max(value[epi_year %in% 2019])) %>%
  ungroup() %>%
  mutate(value_norm=value/peak_2019) %>%
  select(!peak_2019)
...
for (k_age in 1:4) {
  for (k_date in 1:2) {
    for (value_type in c("value","value_norm")) {
      ...
      df_plot <- summ_dyn_all_parsets_broad_age %>%
        filter(varname %in% sel_vars[2] & date>=as.Date("2021-05-01") & date <= lim_dates[k_date] & agegroup_broad %in% sel_agegr & (metric %in% c("median","ci50_low","ci50_up"))) %>% 
        select(!c(varname, !!val_type_excl)) %>%
        pivot_wider(names_from = metric, values_from = !!value_type)
      p <- df_plot %>% 
        ggplot(aes(x = date, group = age_exp_par_bins, color = factor(age_exp_par_bins))) + 
          geom_line(aes(y = median), size=1.1) +
          geom_ribbon(aes(ymin = ci50_low, ymax = ci50_up, fill = factor(age_exp_par_bins)), color=NA, alpha=0.2) + 
      ...
      if (grepl("norm",value_type)) {
        p<-p+geom_hline(yintercept=1,linetype="dashed",size=1/3)
      }; 
      p
      # save
      ...
      if (!dir.exists(here::here(foldername, "dynamics/", abs_val_folder))) {
        dir.create(here::here(foldername, "dynamics/", abs_val_folder))
      }
      plot_fn <- paste0(foldername, "dynamics/", abs_val_folder, "weekly_hosp_by_ageexpdep_", 
        ifelse(grepl("norm", value_type), "norm_2019_peak_",""),
          gsub("-", "_", gsub("<", "below", sel_agegr)), 
          "_", 
          substr(lim_dates, 1, 4)[k_date], ".png")
...
for (k_date in 1:2) {
  sel_agegr <- unique(summ_dyn_all_parsets_broad_age$agegroup_broad)[k_age]
  df_plot <- summ_dyn_all_parsets_broad_age %>%
    filter(varname %in% sel_vars[2] & date >= as.Date("2021-05-01") & !(agegroup_broad %in% "5+y") & date <= lim_dates[k_date] & (metric %in% c("median","ci50_low","ci50_up"))) %>% select(!c(varname,value)) %>%
  ...
  p <- df_plot %>% 
    ggplot(aes(x = date, group = agegroup_broad, color = factor(agegroup_broad))) + 
  ...
  p
  # save
  if (!dir.exists(here::here(foldername, "dynamics/facet_ageexpdep/"))) {
    dir.create(here::here(foldername, "dynamics/facet_ageexpdep/"))
  }
  plot_fn <- paste0(foldername,"dynamics/facet_ageexpdep/", "weekly_hosp_by_ageexpdep_", 
    ifelse(grepl("norm",value_type),"norm_2019_peak_",""),
...
dyn_all_parsets_broad_age %>%
  filter(par_id %in% unique(par_id)[1:11]) %>%
  ggplot(aes(x = date, y = incid_hosp, group = par_id, color = factor(par_id))) +
    geom_line() + facet_wrap(~agegroup_broad, scales = "free_y") + 
