...
# library(ISOweek)
SARIwatch_RSVhosp_under5_2018_2020_weekly_counts <- read_csv("data/SARIwatch_RSVhosp_under5_2018_2020_weekly_counts.csv",
                   col_types="ffddd") %>% mutate(wk_n=gsub("\\.","-W",wk_n),wk_n=factor(wk_n,levels=unique(wk_n)),
...
SARIwatch_RSVhosp_under5_2018_2020_weekly_counts %>% group_by(year) %>% summarise(annual_cumul_rate=sum(rate_under5yrs))
...
# = 822.7/1e5
(4184*sum(rsv_age_groups$value[1])+1272*sum(rsv_age_groups$value[2:4])+114*sum(rsv_age_groups$value[5:7]))/sum(rsv_age_groups$value[1:7])
...
under_report_factor_under5 = mean((SARIwatch_RSVhosp_under5_2018_2020_weekly_counts %>% group_by(year) %>% 
  ...
  mean(c((35.1*sum(rsv_age_groups$value[1:2]) + 5.31*sum(rsv_age_groups$value[3:7]))*100/sum(rsv_age_groups$value[1:7]),
...
### ### ### ### ### ### ### ### ### ### ### ### 
# for 65+y
SARIwatch_RSVhosp_over65_2018_2020_weekly_counts <- read_csv("data/SARIwatch_RSVhosp_over65y_2018_2020_weekly_counts.csv") %>%
...
round(100*abs(sapply(1:n_age, function(x) (sum(ode_sol[1,(x-1)*9+(1:9)+1])-sum(ode_sol[nrow(ode_sol)-1,(x-1)*9+(1:9)+1]) ) ))/
                                          sapply(1:n_age, function(x) sum(ode_sol[1,(x-1)*9+(1:9)+1]) ),3)
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###

# 1/2/3rd infections together
df_cases_infs %>% filter(agegroup<=7) %>% group_by(date) %>% summarise(value=sum(value)) %>% filter(date>as.Date("2014-09-01")) %>%
...
ggplot(aes(x=date,y=value)) + geom_line() +
  ...
  geom_vline(data=df_cases_infs %>% filter(yday(date)==yday("2017-12-05")) %>% ungroup() %>% select(date) %>% unique(),
...
simul_hosp_rate <- left_join(
  df_cases_infs %>% filter(agegroup<=7) %>% #  & date>as.Date("2017-10-01") & date<as.Date("2022-04-01")
  ...
  mutate(hosp_rate_per_100k=hosp/(sum(rsv_age_groups$stationary_popul[1:7])/1e5)) %>% relocate(date,.before=year_week)
...
A_opt[,5] <- qnorm(A_opt[,5],mean=350,sd=50); A_opt[,5] <- 1/A_opt[,5]; A_opt[,6] <- round(qunif(A_opt[,6],min=43,max=48))
...
# foreach(k_n = 1:nrow(A_opt), .combine='c') %dopar%  {
  ...
  attack_rates <- left_join(df_cases_infs %>% filter((date>=as.Date("2019-09-30") & date<as.Date("2020-03-29")) | 
            ...
            # (bc in empirical studies they also count <at least one> infection, baby reinfected in a season would not be counted twice)
                  ...
                  by="agegroup") %>%
                  mutate(attack_rate_stat_pol=cumul_inf/stationary_popul, attack_rate_real_pol=cumul_inf/real_pop,k_par=k_n,
  ...
  attack_rates_aligned_kenya = c( 
    sum(attack_rates$attack_rate_stat_pol[1:2]*attack_rates$stationary_popul[1:2]/sum(attack_rates$stationary_popul[1:2])), # agegroup 1-2
    sum(attack_rates$attack_rate_stat_pol[3:7]*attack_rates$stationary_popul[3:7]/sum(attack_rates$stationary_popul[3:7])),
    attack_rates$attack_rate_stat_pol[8],
    attack_rates$attack_rate_stat_pol[9],
    sum(attack_rates$attack_rate_stat_pol[10:11]*attack_rates$stationary_popul[10:11]/sum(attack_rates$stationary_popul[10:11]))
    )
  AR_log_LLH = dbinom(x=kenya_attack_rates$RSV_sympt_posit,size=kenya_attack_rates$n_test,prob=attack_rates_aligned_kenya,log=T)
  ...
  hosp_daily=left_join(df_cases_infs %>% 
  ...
  # aggreg to weeks
  list_hosp_with_data_weekly[[k_valid]] = hosp_daily %>% mutate(year_week=paste0(isoyear(date),"-", isoweek(date)),
    ...
    mutate(simul_hosp_rate_100k=ifelse(grepl("65y",broad_age),1e5*simul_hosp_sum_full/rsv_age_groups$stationary_popul[11],
...
all_hosp <- left_join(
    ...
    bind_rows(list_hosp_with_data_weekly),
    by="date") %>%
    mutate(simul_hosp_scaled=ifelse(grepl("65y",broad_age),
                            simul_hosp_sum_full*under_report_factor_over65y*(pop_AGE65PLUS/rsv_age_groups$stationary_popul[11]),
                            simul_hosp_sum_full*under_report_factor_under5*(pop_AGEUNDER5/sum(rsv_age_groups$stationary_popul[1:7])) ) ,
...
left_join(all_attack_rates, estim_attack_rates_kenya,by="agegroup_name") %>% 
  mutate(crit1=attack_rate_stat_pol>=sympt_attack_rate/2.5 & attack_rate_stat_pol<=sympt_attack_rate*2.5) %>% group_by(k_par) %>% 
  summarise(sum_crit1=sum(crit1),crit1=(sum_crit1>=9)), # %>% filter(sum_crit1==11)
# crit 2: seasonal concentration (of 5-year olds)
all_hosp %>% filter(broad_age %in% "<5y") %>% mutate(week=as.numeric(gsub("2018-W|2019-W|2020-W","",wk_n))) %>% group_by(k_par) %>%
  summarise(all_cumul=round(sum(simul_cases)),
            in_season_cumul=round(sum(simul_cases[week>=46 | week<=2])),out_season_cumul=round(all_cumul-in_season_cumul),
...
all_hosp %>% mutate(week=as.numeric(gsub("2018-W|2019-W|2020-W","",wk_n))) %>% 
  ...
  summarise(interyr_diff=simul_cases[year %in% "2018-19"]-simul_cases[year %in% "2019-20"],simul_cases=sum(simul_cases)/2) %>%
...
left_join(all_attack_rates %>% select(c(k_par,sum_neg_llh_5y,sum_neg_llh_65y,ALL_neg_log_LLH)) %>% distinct(),all_crit_table) %>% 
