...
estim_attack_rate_filename <- commandArgs(trailingOnly=TRUE)[6]
...
string_main_run <- paste0(paste0(unlist(lapply(1:k_length, # "#!/bin/bash\n",
  function(x) paste0("nohup Rscript --vanilla fcns/parscan_runner_cmd_line.R ",
                     paste0(c(parscan_split[[x]],simul_dur,post_npi,partable_filename,estim_attack_rate_filename),collapse=" "),
...
string_start_run <- paste0("#!/bin/bash\n",paste0(unlist(lapply(1:(k_length-1), 
  function(x) paste0("nohup Rscript --vanilla fcns/parscan_starter_cmd_line.R ",paste0(c(x,k_length,2*simul_dur),collapse=" "),
 " > simul_output/parscan/parallel/nohup_starter_",x,".out",ifelse(x<(k_length-1)," & \n",""),collapse=""))),
 collapse="") )

# write to file
# string_start_run,"PID=$!","wait $PID","Rscript collect_save_initconds.R"
write.table(paste0(c("#!/bin/bash\n",string_main_run,"PID=$!","wait $PID","Rscript fcns/collect_save_fullruns.R"),
