...
for (k in 1:ncol(median_parset)){ 
  ...
  parset_close_to_median <- 
    map2_df(abs(map2_df(results_fullscan_hosp[,colnames(median_parset)[-k]] %>% distinct(),
                        median_parset[,-k],`-`)),
            median_parset[,-k],`/`) %>% # this is calculating relative error (%)
    ...
    pivot_longer(!c(mean_abs_per_error,max_abs_per_error,par_id)) %>% 
    group_by(name) %>% 
  ...
  x_full_scan <- bind_rows(sel_all %>% 
                             summarise(norm_median=median(hosp_tot_norm,na.rm=T),
                                      norm_min=min(hosp_tot_norm,na.rm=T),
                                      norm_max=max(hosp_tot_norm,na.rm=T),
                           ...
                           sel_all %>% 
                             summarise(norm_median=median(peak_hosp_norm,na.rm=T),
                                       norm_min=min(peak_hosp_norm,na.rm=T),
                                       norm_max=max(peak_hosp_norm,na.rm=T),
  ...
  output_ranges_full_scan <- bind_rows(output_ranges_full_scan,
                                       x_full_scan,x_parset_sel)
  # mean age shift 
  x_mean_age <- bind_rows(sel_all %>% 
                    summarise(norm_median=median(mean_age_cumul_hosp_shift,na.rm=T),
                              norm_min=min(mean_age_cumul_hosp_shift,na.rm=T),
                              norm_max=max(mean_age_cumul_hosp_shift,na.rm=T),
