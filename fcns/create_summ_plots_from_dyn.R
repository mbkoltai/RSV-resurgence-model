  ...
  for (k_plot_par in 1:length(sel_pars)) {
    for (k_plot_var in 1:length(sel_varnames)) { # 1:length(sel_varnames)
      ...
      df_plot <- summ_dyn_peak_cumul_meanage_byparvalue %>% 
        ...
        mutate(varname=case_when(grepl("peak_hosp",varname) ~ "peak hospitalisation (number)",
        ...
        mutate(parname=case_when(grepl("age_exp_PC1",parname) ~ "exposure (-1) <-> age (1)", 
                                 grepl("age_exp_ratio",parname) ~ "age- to exposure-dependence ratio", 
      ...
      n_par_value <- length(unique(df_plot$parvalue)); median_width=0.5/100
      # colour palette
      ...
      plot_filename <- gsub("//","/",here(foldername,paste0(subfldr_name,sel_par,
                                      ifelse(start_year==2020,"_incl2020",""),"_epiyear_startwk",epi_year_week_start,".png")))
      # save
