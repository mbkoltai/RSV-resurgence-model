...
for (k_epiyear_start_wk in c(23,40)) {
  for (k_epiyear_plot in 1){ # :3
    ...
    if (length(sel_years)>1) {
      p <- p + geom_col(aes(y=est,x=parname,group=interaction(epi_year,agegroup),color=factor(epi_year),
                            fill=factor(agegroup)),size=2/3,position=position_dodge(width=0.85),width=4/5) +
      ...
      p <- p + geom_col(aes(y=est,x=parname,group=agegroup,fill=factor(agegroup)),size=1/3,color="black",
    ...
    subfldr_name="median_interquant_by_param_value/from_dynamics/PRCC"
    ...
    # SAVE
    ggsave(here(foldername,subfldr_name,filename),width=28,height=20,units="cm")
    message(filename)
    }
  }
}
