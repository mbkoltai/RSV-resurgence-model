...
for (k_age in 1:length(unique(results_fullscan_hosp$agegroup_broad))) {
  ...
  subset_prcc <- results_fullscan_hosp %>% 
    ...
    select(par_id,agegroup_broad,epi_year,exp_dep,age_dep,
  ...
  list_prcc_output[[k_age]] = bind_rows(
    epi.prcc(subset_prcc %>% 
               select(!c(peak_hosp_norm,mean_age_shift_2020,mean_age_shift_2021)),
             sided.test=2,conf.level=0.95) %>% 
      mutate(output="cumulative"), #[,-ncol(subset_prcc)]
    epi.prcc(subset_prcc %>% 
               select(!c(hosp_tot_norm,mean_age_shift_2020,mean_age_shift_2021)),
             sided.test=2,conf.level=0.95) %>% 
      mutate(output="peak"), #[,-ncol(subset_prcc)]
    epi.prcc(subset_prcc %>% 
               select(!c(hosp_tot_norm,peak_hosp_norm,mean_age_shift_2021)),
             sided.test=2,conf.level=0.95) %>% 
      mutate(output="mean_age_shift_2020"),
    epi.prcc(subset_prcc %>% 
               select(!c(hosp_tot_norm,peak_hosp_norm,mean_age_shift_2020)),
             sided.test=2,conf.level=0.95) %>% 
    ...
    mutate(name=case_when(grepl("exp_dep",name) ~ "exposure-dependence", 
                          ...
                          grepl("seasforce_peak",name) ~ "seasonal forcing \n(% above baseline)",
                          grepl("seasforc_width_wks",name) ~ "seasonal forcing \n(duration, weeks)",
                          grepl("R0",name) ~ "R0 (baseline)",
                          grepl("omega",name) ~ "waning rate")) %>%
    mutate(filter_crit=
             ifelse((grepl("mean_age",output) & 
                       agegroup_broad %in% "<1y") | !grepl("mean_age",output),T,F)) %>%
