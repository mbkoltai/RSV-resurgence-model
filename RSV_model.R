...
n_years=3; max_time=n_years*n_days_year; timesteps <- seq(0,max_time,by=elem_time_step)
# seasonal forcing
forcing_vector=fun_seas_forc(timesteps,peak_day=49*7,st_dev_season=27,basal_rate=0.02) # matplot(timesteps,forcing_vector,type="l")
# INITIAL CONDITIONS
...
scale_val='free'; ggplot(df_ode_solution_tidy %>% group_by(t_years,agegroup_name) %>% summarise(agegroup_total=sum(value)),
...
####
# Plot time course --------------------------------------------------------
xval_lims=c(max_time/365-2.25,max_time/365); xval_breaks=seq(0,max_time/365,by=1/4)
...
# PLOT
ggplot(df_ode_solution_tidy[vars_to_show,],aes_string(x="t_years",y=value_type,group="name",color="infection")) +
...
ggsave(timecourse_filename,width=32,height=16,units="cm") }
...
truthvals_rsv=resp_virus_data_uk_tidy$name %in% "RSV" & (!resp_virus_data_uk_tidy$Year %in% c(2014,2020))
resp_virus_data_uk_tidy[,"type"]='dashed'; resp_virus_data_uk_tidy[resp_virus_data_uk_tidy$Year %in% "mean","type"]='solid'
# plot
ggplot(resp_virus_data_uk_tidy[truthvals_rsv,],aes(x=startweek,y=value,group=Year,color=Year,linetype=type)) + 
  geom_line() + geom_point() + facet_wrap(~Age,scales="free") + theme_bw() + standard_theme + ylab("")
