...
# final popul: # sum((df_cases_infs %>% filter(!grepl("newinf",name) & t==max(t)))$value)
...
# plot attack rates
ggplot(sum_inf_epiyear_age %>% mutate(attack_rate_perc=ifelse(epi_year==2020,NA,attack_rate_perc)) %>% filter(epi_year>2016) ) + 
 ...
 facet_wrap(~agegroup_name,scales="free_y") + theme_bw() + standard_theme + xlab("epi_year") + ylab("attack rate % age group") +
  geom_rect(xmin=(2020-2016)-0.5,xmax=(2020-2016)+1/2, # which(unique(sum_inf_epiyear_age$epi_year)==2020)
      ymin=-Inf,ymax=Inf,fill="pink",alpha=0.1) + geom_hline(data=estim_attack_rates %>% pivot_longer(!agegroup_name),
    ...
    labs(caption=paste0("seas peak=",round(max(forcing_vector_npi),2),"x baseline, NPI contact red.=-",
                        round((1-min(forcing_vector_npi))*1e2),"%, R0 (baseline)=",round(R0val,2))) + 
  ggtitle("attack rates by age group",subtitle=paste0("suscept=[",paste0(round(delta_primary,3),collapse=","),"]"))
# save
...
# save
ggsave(paste0("simul_output/",dep_subfolder_name,"seas_share_",filetag,"infs.png"),width=20,height=15,units="cm")
